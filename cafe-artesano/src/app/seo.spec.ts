...
describe('static SEO and deployment artifacts', () => {
  ...
  it('maps physical localized build artifacts to Netlify package-config semantics without a global SPA fallback', () => {
    ...
    const packageCurrentDirectory = packageDirectory;
    const packageBaseDirectory = packageDirectory;
    const rootCurrentDirectory = repositoryRoot;
    const rootBaseDirectory = resolve(rootCurrentDirectory, rootBuildSettings['base'] ?? '');
    const packageEffectiveCommandCwd = packageBaseDirectory;
    const rootEffectiveCommandCwd = rootBaseDirectory;
    const packageEffectivePublishRoot = resolve(packageBaseDirectory, packageBuildSettings['publish'] ?? '');
    const rootEffectivePublishRoot = resolve(rootBaseDirectory, rootBuildSettings['publish'] ?? '');
    ...
    expect(packageCurrentDirectory).toBe(packageDirectory);
    expect(packageBaseDirectory).toBe(packageCurrentDirectory);
    expect(rootBaseDirectory).toBe(packageDirectory);
    expect(packageBuildSettings['command']).toBe(rootBuildSettings['command']);
    expect(packageEffectiveCommandCwd).toBe(rootEffectiveCommandCwd);
    ...
    expect(existsSync('src/_redirects')).toBe(false);
    expect(existsSync('public/_redirects')).toBe(false);
    expect(existsSync('dist/cafe-artesano/browser/_redirects')).toBe(false);
