...
Netlify resolves configuration in this monorepo from the package directory, then the configured base directory, then the repository root. `cafe-artesano/netlify.toml`, beside the Angular `package.json`, is the authoritative configuration: it runs `npm run build` and publishes `dist/cafe-artesano/browser` relative to the package root without declaring `base`. The repository-root `netlify.toml` is only a fallback; its `base = "cafe-artesano"` and the same command and publish value resolve to the same physical directory. Keep both configurations semantically aligned.
