...
Netlify selects configuration by package-directory lookup before applying build-directory semantics. `cafe-artesano/netlify.toml`, beside the Angular `package.json`, is the authoritative configuration. Because it declares no `base`, Netlify runs its command from the repository root and resolves its publish path from that same root; it therefore explicitly runs `cd cafe-artesano && npm run build` and publishes `cafe-artesano/dist/cafe-artesano/browser`. The repository-root `netlify.toml` is only a fallback: its `base = "cafe-artesano"` changes the build working directory and makes its `npm run build` plus `dist/cafe-artesano/browser` publish value resolve to the same physical package and publish directory. Package-directory configuration lookup does not itself set the base directory. Keep both configurations semantically aligned.
